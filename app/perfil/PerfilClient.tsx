      ...
      {isCancelled ? (
        ...
        <div className="flex items-center justify-between relative px-2">
          ...
          <div className="absolute top-4 left-4 right-4 h-[1px] bg-steel/30 z-0">
            <div
              className="h-full bg-gold-action transition-all duration-700"
          ...
          {ORDER_STEPS.map((step, i) => {
            ...
            return (
              <div key={step.key} className="flex flex-col items-center gap-2.5 z-10">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${done ? 'bg-gold-action text-obsidian shadow-lg shadow-gold-action/10' : 'bg-graphite border border-steel/50 text-titanium'
...
export default function PerfilClient({ initialUser, initialProfile, initialOrders, initialAddresses }: {
  ...
  }, [initialUser, supabase.auth, router, syncFavorites])
        ...
        return (
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
            <div className="flex items-center justify-between pb-4 border-b border-steel/10">
              ...
              {!editingData ? (
                <Button variant="outline" size="sm" className="border-steel/50 text-chrome hover:border-gold-action hover:text-gold-action text-xs tracking-wider uppercase font-semibold rounded-none px-4 py-2" onClick={() => setEditingData(true)}>
                  Editar
                </Button>
              ) : (
              ...
              {[
                ...
                <div key={key} className="space-y-2">
                  ...
                  <Input
                    ...
                    className="h-12 bg-graphite border-steel/30 rounded-none text-white-diamond focus:border-gold-action text-sm font-sans"
                  />
                </div>
              ))}
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            {orders.length === 0 ? (
              ...
              <div className="space-y-6">
                {orders.map(order => (
                  <div key={order.id} className="bg-graphite/40 border border-steel/30 rounded-none overflow-hidden hover:border-gold-action/20 transition-all duration-300 shadow-xl">
                    {/* Order Header */}
                    <div className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <p className="text-base font-semibold text-gold-action">{formatPrice(order.total)}</p>
                      </div>
                        ...
                        <div className="mt-6 p-4 rounded-none bg-graphite border border-steel/30 space-y-3">
                          ...
                          {order.tracking_photo_url && (
                            <a href={order.tracking_photo_url} target="_blank" rel="noopener noreferrer"
                              className="inline-flex items-center gap-1.5 text-xs font-medium text-gold-action hover:underline mt-1">
                          ...
                          {order.admin_note && (
                            <div className="flex items-start gap-2 mt-2 pt-2 border-t border-steel/10">
                              <MessageSquare className="w-3.5 h-3.5 text-gold-action shrink-0 mt-0.5" />
                        ...
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                          ...
                          {order.items?.map((item, i) => (
                            <div key={i} className="flex justify-between items-center py-2 first:pt-0 border-b border-steel/5 last:border-b-0">
                              ...
                              <p className="text-sm font-medium text-gold-action">{formatPrice(item.price * item.quantity)}</p>
                            </div>
                          ))}
                          <div className="pt-4 border-t border-steel/20 space-y-1.5 text-xs font-light">
                            ...
                            <div className="flex justify-between font-semibold text-sm pt-2 border-t border-steel/10 text-white-diamond"><span>Total</span><span className="text-gold-action font-bold">{formatPrice(order.total)}</span></div>
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
              ...
              <div className="space-y-4">
                {notifications.map(notif => (
                  <motion.div key={notif.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                    className={`p-6 rounded-none border transition-colors ${notif.read ? 'bg-carbon/50 border-steel/20' : 'bg-carbon border-gold-action/30 shadow-lg'}`}>
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            {favorites.length === 0 ? (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {favorites.map(item => (
                  <div key={item.id} className="bg-graphite/40 border border-steel/30 rounded-none overflow-hidden hover:border-gold-action/30 transition-all duration-350 group shadow-lg">
                    ...
                    <div className="p-4">
                      ...
                      <p className="text-gold-action font-semibold text-sm mt-1">{formatPrice(item.price)}</p>
        ...
        return (
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            {addresses.length === 0 && !showAddressForm ? (
              ...
              <div className="space-y-4">
                {addresses.map(addr => (
                  <div key={addr.id} className="bg-graphite/40 border border-steel/30 rounded-none p-6 hover:border-gold-action/30 transition-colors shadow-lg">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        {addr.is_default && <span className="inline-flex items-center gap-1 px-3 py-1 bg-gold-action/10 border border-gold-action/30 text-gold-action text-[9px] uppercase tracking-wider rounded-none font-semibold mb-3"><Check className="w-3 h-3" /> Principal</span>}
        ...
        <div className="container mx-auto px-4 max-w-7xl">
          <div className="grid lg:grid-cols-4 gap-8">
                ...
                <div className="text-center">
                  <div className="relative inline-block mb-4">
                    <div className="w-24 h-24 rounded-none border border-steel/30 bg-graphite flex items-center justify-center overflow-hidden">
                      {userData.avatarUrl ? <Image src={userData.avatarUrl} alt="avatar" width={96} height={96} className="object-cover" /> : <User className="w-12 h-12 text-gold-action" />}
                    </div>
                    <button className="absolute bottom-0 right-0 w-8 h-8 bg-gold-action flex items-center justify-center text-obsidian hover:bg-gold-action/90 transition-colors">
                  ...
                  <div className="flex items-center justify-center gap-2 mt-4 text-[10px] uppercase tracking-wider text-gold-action font-sans font-medium bg-graphite/40 py-1.5 px-3 border border-steel/20">
                  ...
                  {menuItems.map(item => (
                    <button key={item.id} onClick={() => setActiveSection(item.id)}
                      className={`w-full flex items-center gap-3 px-4 py-3 rounded-none transition-all ${activeSection === item.id
                          ? 'bg-gold-action text-obsidian font-semibold'
            ...
            <div className="lg:col-span-3">
              <div className="bg-carbon border border-steel/30 rounded-none p-6 sm:p-8 shadow-2xl">
                {renderContent()}
              </div>
            </div>
          </div>
